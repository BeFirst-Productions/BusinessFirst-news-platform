...
const RATIO_OPTIONS_BY_PAGE: Record<string, any[]> = {
  home: [
    ...
    { value: 'ad_2', label: 'Ad 2', ratio: 'Portrait', aspectClass: 'aspect-[4/5] h-12', desc: 'Sidebar Top', dimensions: '240 × 300 px' },
    { value: 'ad_3', label: 'Ad 3', ratio: 'Portrait', aspectClass: 'aspect-[4/5] h-12', desc: 'Sidebar Bottom', dimensions: '240 × 300 px' },
    { value: 'ad_4', label: 'Ad 4', ratio: 'Billboard', aspectClass: 'aspect-[16/3] w-full max-w-[120px]', desc: 'Middle Page', dimensions: '1600 × 300 px' },
    { value: 'ad_5', label: 'Ad 5', ratio: 'Medium Rectangle', aspectClass: 'aspect-[12/5] h-12', desc: 'Technology Section', dimensions: '600 × 250 px' },
    ...
    <div className="space-y-6 max-w-3xl">
      ...
      <form onSubmit={handleSubmit((data) => createAd.mutate(data))} className="space-y-6">
                  ...
                  render={({ field }) => (
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                      {currentRatioOptions.map((option) => {
                        const isOccupied = occupiedRatios.includes(option.value);
                        return (
                          <div
                            key={option.value}
        ...
        <div className="flex justify-end gap-3">
          <Button type="button" variant="outline" onClick={() => router.back()} disabled={isNavigating || createAd.isPending || (createAd as any).isLoading}>
            Cancel
          </Button>
          <Button
            type="button"
          ...
          <Button type="submit" loading={isNavigating || createAd.isPending || (createAd as any).isLoading} leftIcon={<Save className="h-4 w-4" />}>
            Create Ad
          </Button>
        </div>
      </form>
